...
class ReferenceImageSelector:
    ...
    async def select_reference_images_and_generate_prompt(
        ...
        reference_limit = _reference_limit(max_reference_images)
        ...
        if len(available_image_path_and_text_pairs) >= 8:
            ...
            messages = [
                SystemMessage(
                    content=system_prompt_template_select_reference_images_only_text.format(
            ...
            try:
                ...
                ref_indices = _bounded_indices(ref.ref_image_indices, len(available_image_path_and_text_pairs), reference_limit)
                filtered_image_path_and_text_pairs = [available_image_path_and_text_pairs[i] for i in ref_indices]
                logging.info(f"Filtered image idx:{ref_indices}")
        ...
        messages = [
            SystemMessage(
                content=system_prompt_template_select_reference_images_multimodal.format(
                    format_instructions=parser.get_format_instructions(),
                    max_reference_images=reference_limit,
                )
            ),
        ...
        try:
            ...
            ref_indices = _bounded_indices(response.ref_image_indices, len(filtered_image_path_and_text_pairs), reference_limit)
            reference_image_path_and_text_pairs = [filtered_image_path_and_text_pairs[i] for i in ref_indices]
            return {
...
def _reference_limit(max_reference_images: Optional[int]) -> int:
    if max_reference_images is None or max_reference_images <= 0:
        return 8
    return max_reference_images


def _bounded_indices(indices: List[int], item_count: int, limit: int) -> List[int]:
    bounded: List[int] = []
    for idx in indices:
        if 0 <= idx < item_count and idx not in bounded:
            bounded.append(idx)
