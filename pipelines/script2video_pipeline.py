...
def _max_reference_images(image_generator: Any) -> Optional[int]:
    value = getattr(image_generator, "max_reference_images", None)
    if value is None:
        return None
    try:
        limit = int(value)
...
def _reference_pair_count(selector_output: Dict[str, Any]) -> int:
    pairs = selector_output.get("reference_image_path_and_text_pairs")
    return len(pairs) if isinstance(pairs, list) else 0


def _normalize_reference_selector_output(
    selector_output: Dict[str, Any],
    frame_description: str,
    max_reference_images: Optional[int],
) -> Dict[str, Any]:
    if max_reference_images is None:
        return selector_output
    pairs = selector_output.get("reference_image_path_and_text_pairs")
    if not isinstance(pairs, list) or len(pairs) <= max_reference_images:
        return selector_output

    limited_pairs = pairs[:max_reference_images]
    max_index = max_reference_images - 1
    prompt = (
        "Create an image matching this frame description:\n"
        f"{frame_description}\n"
        f"Use only the provided reference images listed above, from Image 0 to Image {max_index}. "
        "Do not refer to any image outside that range."
    )
    normalized = dict(selector_output)
    normalized["reference_image_path_and_text_pairs"] = limited_pairs
    normalized["text_prompt"] = prompt
    return normalized
    ...
    async def generate_frames_for_single_camera(
        ...
        if os.path.exists(first_shot_ff_path):
            ...
            if camera.parent_shot_idx is None or camera.missing_info is not None:
                ...
                max_reference_images = _max_reference_images(self.image_generator)
                if os.path.exists(ff_selector_output_path):
                    ...
                    original_count = _reference_pair_count(ff_selector_output)
                    ff_selector_output = _normalize_reference_selector_output(
                        ff_selector_output,
                        shot_descriptions[first_shot_idx].ff_desc,
                        max_reference_images,
                    )
                    if _reference_pair_count(ff_selector_output) != original_count:
                        with open(ff_selector_output_path, 'w', encoding='utf-8') as f:
                            json.dump(ff_selector_output, f, ensure_ascii=False, indent=4)
                        _emit_render_progress(progress, "frame_prompt_limited", "Reference images limited for image generator", {"camera_idx": camera.idx, "shot_idx": first_shot_idx, "frame_type": "first_frame", "max_reference_images": max_reference_images})
                else:
    ...
    async def generate_frame_for_single_shot(
        ...
        if os.path.exists(frame_image_path):
            ...
            if os.path.exists(selector_output_path):
                ...
                original_count = _reference_pair_count(selector_output)
                selector_output = _normalize_reference_selector_output(
                    selector_output,
                    frame_desc,
                    max_reference_images,
                )
                if _reference_pair_count(selector_output) != original_count:
                    with open(selector_output_path, 'w', encoding='utf-8') as f:
                        json.dump(selector_output, f, ensure_ascii=False, indent=4)
                    _emit_render_progress(progress, "frame_prompt_limited", "Reference images limited for image generator", {"shot_idx": shot_idx, "frame_type": frame_type, "max_reference_images": max_reference_images})
            else:
